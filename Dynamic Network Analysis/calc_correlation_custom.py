#!/usr/bin/env python
#
# Calculate dynamic network analysis correlation in spike protein MD trajectory
#
# Requires the package MD-TASK available at https://github.com/RUBi-ZA/MD-TASK
#
# Usage
# Place calc_correlation_custom.py file in the MD-TASK folder 
# activate the environment using source ./MD-TASK/venv/bin/activate and run using 
# python ./MD-TASK/calc_correlation_custom.py --trajectory <traj_file.dcd> --topology <structure_file.pdb> --step <Number of frames to skip> --title <Title of the plot> --prefix <Name of the correlation ouput file> 
...
def plot_allres_map(correlation, t_mat, title, output_prefix):
    """
        Custom plot script for the full trimer.
        Requires renumbering protomer B and C in the .psf to avoid conflicts.
    """
    M = np.array(correlation)
    res = np.array(list(t_mat),dtype=int) 
    max_res = res.max()
    Z = np.empty((max_res+1,max_res+1))
    Z.fill(np.nan)  
...
def main(args):
    ...
    plot_allres_map(correlation, traj_matrix, args.title, args.prefix)
